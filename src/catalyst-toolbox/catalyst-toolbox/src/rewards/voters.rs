...
fn rewards_to_mainnet_addresses(
    ...
    for (addr, reward) in rewards {
        ...
        for c in contributions {
            let rewards_to_add = reward * Rewards::from(c.value) / total_value;
            let key = match addr_kind {
                AddrKind::Stake => c.stake_public_key.clone(),
                AddrKind::Reward => c.reward_address.clone(),
            };
            *res.entry(key).or_default() += rewards_to_add;
        }
    }
...
pub fn calc_voter_rewards(
    ...
    Ok(rewards_to_mainnet_addresses(
        rewards,
        active_addresses,
        addr_kind,
    ))
